...
void TalkScreenBase::Load()
{
	...
	_talkbox.SetCoordinates(0, 0);
		
	_fwdbtn.Load(FORWARD_BUTTON_IMG, FORWARD_BUTTON_IMG);
	_fwdbtn.SetGlowImage(FORWARD_HOVER_IMG);
	...
	_fwdbtn.SetCoordinates(1660, 930);
...
{
	_talkbox.Draw();
	SDLGX_DrawTextCenter(360, 765, _name.c_str(), FONT_MYRIADPRO_REGULAR, 50, { 255,255,255 });
	...
	while (i <= current_split_pos - 1) {
		...
		SDLGX_DrawText(startX, startY, text.c_str(), FONT_MYRIADPRO_REGULAR, 50, { 255,255,255 });
		startY += 60;
		i++;
	}
