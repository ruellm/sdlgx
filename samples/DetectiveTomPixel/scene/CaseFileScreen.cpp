...
void CaseFileBoxItem::OnUpdate(float elapsed)
{
	...
	ZoomButton::OnUpdate(elapsed);
}

void CaseFileBoxItem::OnDraw()
{
	if (_selected) {
		SDL_Rect rect = { GetNewX() - 2, GetNewY() - 2, _width + 4, _height + 4 };
...
void CaseFileScreen::OnInitialize()
{
	...
	_backBtn.Load(BACK_BUTTON_IMG);
	_backBtn.SetID(0);
	_backBtn.SetHandler(this);
	_backBtn.SetCoordinates(8, LOGICAL_HEIGHT - 
		(46/*iamge height*/));
	_uimanager->Add(&_backBtn);

	_arrow_right.Load(CHOICE_SCROLL_RIGHT_IMG);
	_arrow_right.SetID(1);
	_arrow_right.SetHandler(this);
	_arrow_right.SetCoordinates(590, 232);
	_arrow_right.SetHoldThreshold(0.1f);
	_uimanager->Add(&_arrow_right);

	_arrow_left.Load(CHOICE_SCROLL_LEFT_IMG);
	_arrow_left.SetID(2);
	_arrow_left.SetHandler(this);
	_arrow_left.SetCoordinates(25,232);
	...
	_clues_btn.Load(CLUES_BUTTON_IMG);
	_clues_btn.SetID(3);
	_clues_btn.SetHandler(this);
	_clues_btn.SetCoordinates(345, 168);
	...
	_show_btn.Load(SHOW_BUTTON_IMG);
	_show_btn.SetID(5);
	_show_btn.SetHandler(this);
	_show_btn._visible = true;
	_show_btn.SetCoordinates(67, 161);
	...
	_scroll_box.Init();
	_scroll_box.SetCoordinates(208,43);	
...
{
	_bg.Draw((LOGICAL_WIDTH/2) - (_bg.GetWidth()/2),
		(LOGICAL_HEIGHT / 2) - (_bg.GetHeight() / 2) - 23);

	//_fg.Draw(0, 80);
	_listmanager.OnDraw();
	_uimanager->OnDraw();

	std::string strText = "Clues";
	if (_mode == CASE_FILE_MODE_PROFILE) {
		strText = "Profile";
	}
	
	/*************************************************************/
	// This is hack solution for zoomed button with text, it should be a class
	int x = _clues_btn.GetNewX() + _clues_btn.GetWidth()/2;
	int y = _clues_btn.GetNewY() + _clues_btn.GetHeight()/2;
	int font_size = 12;
	if (_clues_btn.GetState() == 1)
		font_size *= _clues_btn.GetScaleUp();

	Texture* t = SDLGX_GetTextCenter(x, y, strText.c_str(), FONT_MANASPACE, font_size, { 255,255,255 });
	...
	if (_list.size() == 0) {
		Texture* t = SDLGX_GetTextCenter(LOGICAL_WIDTH / 2, 250, "List is empty", FONT_MANASPACE, 10, { 255,255,255 });
		t->SetBlendMode(SDL_BLENDMODE_BLEND);
		t->SetAlpha(200);
		t->Draw();
		SAFE_DELETE(t);
